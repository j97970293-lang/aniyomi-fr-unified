package com.frunified.rhino.resources
...
/**
 * Messages d'erreur / avertissements de Rhino, embarqués sous forme de classe.
 *
 * POURQUOI : Rhino 1.9.1 lit ses messages dans
 * `com/frunified/rhino/resources/Messages.properties` via
 * `ResourceBundle.getBundle(...)`. Un plugin CloudStream ne contient qu'un
 * `classes.dex` : aucun fichier `.properties` ne peut y être transporté, donc
 * côté téléphone le bundle est introuvable et **chaque** message — même un
 * simple avertissement (`warnAboutNonJSObject`) — lève une
 * `MissingResourceException`, ce qui tuait tous les scrapeurs Nuvio
 * (« ✗ interne: can't find bundle for base name … ») alors que tout
 * fonctionnait sur JVM (jar complet).
 *
 * On fournit donc le bundle sous forme de classe compilée (déxée avec le
 * reste) : `ResourceBundle.getBundle()` la trouve comme n'importe quel bundle.
 *
 * Contenu : copie de `Messages.properties` (Mozilla Public License 2.0, Rhino
 * 1.9.1), relogé `org.mozilla.javascript` → `com.frunified.rhino`.
 */
internal object RhinoMessageData {

    private val messages: Map<String, String> by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
        val out = HashMap<String, String>(420)
    ...
    fun get(key: String): String? = messages[key]

    fun keys(): Set<String> = messages.keys

    /** Nombre de messages disponibles (diagnostic). */
    fun size(): Int = messages.size

    private const val RAW = """implementation.version=@IMPLEMENTATION.VERSION@
params.omit.non.js.object.warning=true
msg.non.js.object.warning=RHINO USAGE WARNING: Missed Context.javaToJS() conversion: Rhino runtime detected object "{0}" of class "{1}" where it expected String, Number, Boolean or Scriptable instance. Please check your code for missing Context.javaToJS() call.
...
msg.incompat.call.details=Method "{0}" called on incompatible object ({1} is not an instance of {2}).
...
msg.method.missing.parameter={0}: At least {1} arguments required, but only {2} passed
msg.bad.for.in.lhs=Invalid left-hand side of for..in loop.
msg.mult.index=Only one variable allowed in for..in loop.
msg.bad.for.in.destruct=Left hand side of for..in loop must be an array of length 2 to accept key/value pair.
msg.bad.computed.property.in.destruct=Unsupported computed property in destructuring.
...
msg.nullish.bad.token=Syntax Error: Unexpected token.
...
msg.only.from.new="Constructor {0}" may only be invoked from a "new" expression.
...
msg.no.empty.interface.conversion=Cannot convert function to interface {0} with no methods
msg.no.function.interface.conversion=Cannot convert function to interface {0} since it contains methods with different names
msg.undefined.function.interface=Property "{0}" is not defined in interface adapter
msg.not.function.interface=Property "{0}" is not a function in interface adapter
...
msg.max.lt.min=Invalid regular expression: The quantifier maximum ''{0}'' is less than the minimum ''{1}''.
...
msg.str.match.all.no.global.flag=String.prototype.matchAll called with a non-global RegExp argument
msg.str.replace.all.no.global.flag=replaceAll must be called with a global RegExp
msg.invalid.group.name=Invalid capture group name
msg.duplicate.group.name=Duplicate capture group name "{0}"
msg.invalid.named.backref=Invalid named capture referenced
msg.lone.quantifier.bracket=Lone quantifier brackets
msg.invalid.class=Invalid character class
msg.invalid.property=Invalid property name
msg.invalid.date=Date is invalid.
msg.toisostring.must.return.primitive=toISOString must return a primitive value, but instead returned "{0}"
msg.invalid.toprimitive.hint=[Symbol.toPrimitive]: expected "string", "number", or "default", but got "{0}"
msg.json.cant.serialize=Do not know how to serialize a {0}
msg.got.syntax.errors=Compilation produced {0} syntax errors.
msg.var.redecl=redeclaration of var {0}.
msg.const.redecl=redeclaration of const {0}.
msg.let.redecl=redeclaration of variable {0}.
msg.parm.redecl=redeclaration of formal parameter {0}.
msg.fn.redecl=redeclaration of function {0}.
msg.let.decl.not.in.block=let declaration not directly within block
msg.bad.object.init=invalid object initializer
...
msg.parm.after.rest=parameter after rest parameter
msg.no.object.rest=object rest properties in destructuring are not supported
...
msg.in.after.for.name=missing in after for
...
msg.no.with.strict=with statements not allowed in strict mode
...
msg.reserved.id=identifier is a reserved word: {0}
msg.super.shorthand.function=super should be inside a shorthand function
msg.optional.super=super is not allowed in an optional chaining expression
msg.super.delete=cannot delete a super property
...
msg.catch.destructuring.requires.es6=Destructuring in catch blocks requires ES6 or later
...
msg.generator.returns=generator function {0} returns a value
msg.anon.generator.returns=anonymous generator function returns a value
msg.syntax.invalid.assignment.lhs=syntax error: Invalid left-hand side in assignment
...
msg.trailing.array.comma=Trailing comma in array literal has different cross-browser behavior
...
msg.default.args=Default values are only supported in version >= 200
msg.no.old.octal.strict=Old octal numbers prohibited in strict mode.
msg.no.old.octal.bigint=Old octal numbers prohibited in BigInt.
...
msg.bad.id.strict="{0}" is not a valid identifier for this use in strict mode.
msg.no.unary.expr.on.left.exp="{0}" is not allowed on the left-hand side of "**".
...
msg.not.iterable={0} is not iterable
msg.invalid.for.each=invalid for each loop
...
msg.set.prop.no.setter=Cannot set property {0} that has only a getter to value ''{1}''.
...
msg.cant.convert.to.bigint=Cannot convert {0} to an BigInt.
msg.cant.convert.to.bigint.isnt.integer=Cannot convert {0} to an BigInt. It isn\\'t an integer.
msg.bigint.bad.form=illegally formed BigInt syntax
...
msg.instanceof.bad.target=Target of ``instanceof`` must be callable or have ``[Symbol.hasInstance]`` method.
msg.instanceof.bad.prototype=''prototype'' property of ''{0}'' is not an object.
msg.in.not.object=Can''t use ''in'' on a non-object.
msg.bad.radix=illegal radix {0}.
msg.division.zero=Division by zero.
msg.bigint.negative.exponent=BigInt negative exponent.
msg.bigint.out.of.range.arithmetic=BigInt is too large.
msg.cant.convert.to.primitive=Cannot convert object to primitive value.
msg.out.of.range.index=Index {0} out of range
...
msg.both.data.and.accessor.desc=Cannot be both a data and an accessor descriptor.
msg.change.configurable.false.to.true=Cannot change the configurable attribute of "{0}" from false to true.
msg.change.enumerable.with.configurable.false=Cannot change the enumerable attribute of "{0}" because configurable is false.
msg.change.writable.false.to.true.with.configurable.false=Cannot change the writable attribute of "{0}" from false to true because configurable is false.
msg.change.value.with.writable.false=Cannot change the value of attribute "{0}" because writable is false.
msg.change.getter.with.configurable.false=Cannot change the get attribute of "{0}" because configurable is false.
msg.change.setter.with.configurable.false=Cannot change the set attribute of "{0}" because configurable is false.
msg.change.property.data.to.accessor.with.configurable.false=Cannot change "{0}" from a data property to an accessor property because configurable is false.
msg.change.property.accessor.to.data.with.configurable.false=Cannot change "{0}" from an accessor property to a data property because configurable is false.
msg.not.extensible=Cannot add properties to this object because extensible is false.
msg.delete.property.with.configurable.false=Cannot delete "{0}" property because configurable is false.
...
msg.invalid.re.flag=invalid flag ''{0}'' after regular expression
msg.no.re.input.for=no input for {0}
msg.illegal.character=illegal character: {0}
...
msg.empty.array.reduce=Reduce of empty array with no initial value
...
msg.generator.executing=The generator is still executing from a previous invocation.
msg.yield.closing=Yield from closing generator
msg.called.null.or.undefined={0}.prototype.{1} method called on null or undefined
msg.first.arg.not.regexp=First argument to {0}.prototype.{1} must not be a regular expression
msg.arrowfunction.generator=arrow function can not become generator
msg.arguments.not.access.strict=Cannot access "{0}" property of the arguments object in strict mode.
msg.object.cyclic.prototype=Cyclic prototype "{0}" value not allowed.
msg.default.args.use.strict=A function cannot have "use strict" directive with default arguments
msg.object.not.symbolscriptable=Object {0} does not support Symbol keys
msg.no.assign.symbol.strict=Symbol objects may not be assigned properties in strict mode
msg.not.a.string=The object is not a string
msg.not.a.number=The object is not a number
msg.cant.convert.to.number=Cannot convert {0} to a number
msg.no.symbol.new=Symbol objects may not be constructed using \\"new\\"
msg.compare.symbol=Symbol objects may not be compared
msg.no.new={0} objects may not be constructed using \\"new\\"
msg.map.function.not=Map function is not actually a function
msg.constructor.no.function=The constructor for {0} may not be invoked as a function
msg.this.not.instance=\\"this\\" is not an instance of class {0}
msg.function.expected=Expecting the first argument to be a function
msg.constructor.expected=Expecting the first argument to be a constructor
msg.promise.self.resolution=Promise is being self-resolved
msg.promise.capability.state=Invalid promise capability state
msg.promise.all.toobig=Too many inputs to Promise.all
msg.promise.any.toobig=Too many inputs to Promise.any
msg.typed.array.receiver.incompatible=Method %TypedArray%.{0} called on incompatible receiver
msg.typed.array.bad.offset=offset {0} out of range
msg.typed.array.bad.length=length {0} out of range
msg.typed.array.bad.offset.byte.size=offset {0} must be a multiple of the byte size {1}
msg.typed.array.bad.buffer.length.byte.size=used buffer length {0} must be a multiple of the byte size {1}
msg.typed.array.bad.source.array=source array is too long
msg.typed.array.index.out.of.bounds=index {0} is out of bounds [{1}..{2}]
msg.typed.array.out.of.bounds=TypedArray is out of bounds
msg.typed.array.type.mismatch=Typed arrays content type does not match
msg.typed.array.length.too.small=Derived TypedArray constructor created an array which was too small
msg.iterable.expected=Expected the first argument to be iterable
msg.species.invalid.ctor=The [Symbol.species] property of object's constructor is not a constructor
msg.arraybuf.smaller.len=Expected ArrayBuffer with at least {0} bytes, but got {1} bytes instead
msg.arraybuf.same=Expected different ArrayBuffer
msg.arraybuf.detached=Expected ArrayBuffer to not be detached
msg.dataview.bounds=DataView is out of bounds
msg.dataview.offset.range=DataView offset is out of range
msg.dataview.length.range=DataView length is out of range"""
}

/**
 * Bundle demandé par Rhino :
 * `ResourceBundle.getBundle("com.frunified.rhino.resources.Messages")`.
 * `handleGetObject` ne doit jamais renvoyer `null` (sinon MissingResourceException) :
 * à défaut de message connu on renvoie la clé elle-même.
 */
class Messages : ResourceBundle() {
    override fun handleGetObject(key: String): Any? = RhinoMessageData.get(key) ?: key
    override fun getKeys(): Enumeration<String> = Collections.enumeration(RhinoMessageData.keys())
}

/**
 * Variante minuscule : selon le chemin de code, le nom de base demandé est
 * `…resources.Messages` ou `…resources.message`. On expose les deux pour être
 * sûr d'être trouvé dans tous les cas.
 */
@Suppress("ktlint:standard:class-naming")
class message : ResourceBundle() {
    override fun handleGetObject(key: String): Any? = RhinoMessageData.get(key) ?: key
    override fun getKeys(): Enumeration<String> = Collections.enumeration(RhinoMessageData.keys())
}
